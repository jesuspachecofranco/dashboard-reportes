...
with tab4:
    ...
    st.markdown(
        "Identifica rápidamente clientes o direcciones con múltiples reportes aplicando filtros opcionales por fecha."
    )
    
    if "df" in locals() and not df.empty:
        df_analisis = df.copy()
    
        # 1. Filtro Opcional de Rango de Fechas
        usar_filtro_fecha = st.checkbox("Filtrar por rango de fechas de recepción", value=False, key="check_filtro_repetidas")
        
        if usar_filtro_fecha and "RECEPCION_DT" in df_analisis.columns:
            min_f = df_analisis["RECEPCION_DT"].min()
            max_f = df_analisis["RECEPCION_DT"].max()
            
            col_f1, col_f2 = st.columns(2)
            with col_f1:
                f_inicio = st.date_input("Fecha Inicio", value=min_f.date() if pd.notna(min_f) else None, key="rep_f_ini")
            with col_f2:
                f_fin = st.date_input("Fecha Fin", value=max_f.date() if pd.notna(max_f) else None, key="rep_f_fin")
                
            if f_inicio and f_fin:
                df_analisis = df_analisis[
                    (df_analisis["RECEPCION_DT"].dt.date >= f_inicio) & 
                    (df_analisis["RECEPCION_DT"].dt.date <= f_fin)
                ]
    
        # 2. Selector para elegir el criterio de agrupación (En Mayúsculas)
        criterio = st.selectbox(
            "Agrupar incidencias repetidas por:",
            ["CLIENTE", "DIRECCIÓN"],
            key="select_criterio_rep"
        )
        
        columna_grupo = criterio # Ya está en mayúsculas igual que las columnas del DataFrame
        
        if columna_grupo in df_analisis.columns:
            # Limpiamos valores vacíos o nulos para evitar agrupar espacios en blanco
            df_analisis[columna_grupo] = df_analisis[columna_grupo].astype(str).str.strip()
            df_analisis = df_analisis[df_analisis[columna_grupo].isin(["", "NAN", "NONE", "NAT"]) == False]
    
            # 3. Contar cuántas incidencias hay por cada cliente/dirección
            conteo = df_analisis[columna_grupo].value_counts().reset_index()
            ...
            if not repetidos.empty:
                st.warning(f"Se encontraron **{len(repetidos)}** {criterio.lower()}s con más de una incidencia en este periodo.")
                
                # Mostrar tabla resumen general
                st.dataframe(repetidos, use_container_width=True, hide_index=True)
                
                st.markdown(f"#### 📂 Detalle por cada {criterio}")
                
                # 4. Mostrar un expander interactivo para cada grupo repetido
                for index, row in repetidos.iterrows():
                    ...
                    with st.expander(f"📌 {criterio}: {valor_grupo} ({total} incidencias)"):
                        # Filtrar el DataFrame para este grupo específico
                        detalle_grupo = df_analisis[df_analisis[columna_grupo] == valor_grupo]
                        
                        # Columnas en MAYÚSCULAS para que coincidan con tu base de datos y se muestren bien
                        columnas_mostrar = [
                            c for c in [
                                "INCIDENCIA", "RECEPCIÓN", "MOTIVO", "ESTADO", "ZONA", 
                                "TEC", "TCR", "ORIGEN_TIPO"
                            ] if c in df_analisis.columns
                        ]
                        
                        # Mostrar la tabla con los detalles corregidos
                        st.dataframe(
                ...
                st.success(f"No se encontraron {criterio.lower()}s con incidencias repetidas para los filtros seleccionados.")
        else:
            st.error(f"La columna '{columna_grupo}' no se encuentra disponible en los datos.")
    else:
